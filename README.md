...
can be used to list all subject-object pairs linked by the `rdf:type` predicate. Note the use of the qualified name in the predicate; prefixes must match the ones declare in the RDF source data. Alternatively, the unqualified IRI `http://www.w3.org/1999/02/22-rdf-syntax-ns#` may also be used.
...
## License
The `basex-rdf` module is licensed under GPLv3.
Parsers generated by the REx Parser Generator are supplied under the Apache 2.0 license.
